...
fn run() -> Result<(), String> {
    ...
    let url = args
        .next()
        .ok_or("usage: qasset_e2e <gateway_url> <container_hex_file> [mint] [transfer]")?;
    let container_file = args
        .next()
        .ok_or("usage: qasset_e2e <gateway_url> <container_hex_file> [mint] [transfer]")?;
    let mint_amount: u64 = args
        .next()
        .map(|s| s.parse().unwrap_or(1_000_000))
        .unwrap_or(1_000_000);
    let transfer_amount: u64 = args
        .next()
        .map(|s| s.parse().unwrap_or(250_000))
        .unwrap_or(250_000);
    ...
    for (i, want) in owner_id.chunks(8).enumerate() {
        ...
        if got != u64::from_be_bytes(want.try_into().unwrap()) {
            return Err(format!(
                "owner slot {i} is {got}, not the deployer address word"
            ));
        }
    }
    ...
    if supply0 != 0 {
        return Err(format!(
            "the initial supply is {supply0}, expected the deploy parameter 0"
        ));
    }
    conservation(
        &client,
    ...
    let (mint_tx, mint_out, mint_order) = client.call_typed_order(
        ...
        &[
            FieldArg {
                offset: MINT_AMOUNT_OFF,
                value: FieldValue::Word(mint_amount),
            },
            FieldArg {
                offset: MINT_AMOUNT_HI_OFF,
                value: FieldValue::Word(0),
            },
            FieldArg {
                offset: MINT_TO_OFF,
                value: FieldValue::Address(owner_id),
            },
        ],
        &DEPLOYER_SEED,
        0,
    ...
    println!(
        "\n[mint] tx {} finalised at height {mint_height}",
        mint_tx.tx_id
    );
    println!(
        "[mint] owner signed order, signer {}, nonce {}",
        hex(&mint_order.signer),
        mint_order.nonce
    );
    ...
    if supply1 != supply0 + mint_amount || bal_owner1 != mint_amount {
        return Err(format!(
            "mint did not credit: supply {supply1}, balance {bal_owner1}, expected {mint_amount}"
        ));
    }
    let minted = find_event(&client, &contract, MINTED_SELECTOR, mint_height)?
        .ok_or("the mint recorded no Minted event")?;
    let minted_amount = event_amount(&minted, 1)?;
    println!("[mint] Minted event amount {minted_amount}");
    if minted_amount != mint_amount {
        return Err(format!(
            "the Minted event carried {minted_amount}, expected {mint_amount}"
        ));
    }
    ...
    let xfer_args = qcore::contract::build_call_args(
        ...
        &[
            FieldArg {
                offset: XFER_TO_OFF,
                value: FieldValue::Address(holder2_id),
            },
            FieldArg {
                offset: XFER_AMOUNT_OFF,
                value: FieldValue::Word(transfer_amount),
            },
        ],
    )?;
    ...
    println!(
        "\n[transfer] tx {} finalised at height {xfer_height}",
        xfer_tx.tx_id
    );
    ...
    if bal_owner2 != mint_amount - transfer_amount || bal_holder2 != transfer_amount {
        return Err(format!(
            "transfer did not move balance: holder1 {bal_owner2}, holder2 {bal_holder2}"
        ));
    }
    let transferred = find_event(&client, &contract, TRANSFERRED_SELECTOR, xfer_height)?
        .ok_or("the transfer recorded no Transferred event")?;
    ...
    if xfer_evt_amount != transfer_amount {
        return Err(format!(
            "the Transferred event carried {xfer_evt_amount}, expected {transfer_amount}"
        ));
    }
    conservation(
        &client,
    ...
    println!(
        "[transfer] PROOF: balance moved holder1 -> holder2, sum of balances still equals supply"
    );
    ...
    println!(
        "\n[non-owner mint] tx {} finalised at height {wrong_height}, signer {}",
        wrong_tx.tx_id,
    ...
    if supply_w != supply2 {
        return Err(format!(
            "a non owner mint changed supply from {supply2} to {supply_w}, the binding failed"
        ));
    }
    ...
    if supply_r != supply2 {
        return Err(format!(
            "a replayed mint changed supply from {supply2} to {supply_r}, the nonce failed"
        ));
    }
    ...
    let over_args = qcore::contract::build_call_args(
        ...
        &[
            ...
            FieldArg {
                offset: XFER_AMOUNT_OFF,
                value: FieldValue::Word(overdraw),
            },
        ],
    )?;
    ...
    if find_event(&client, &contract, TRANSFERRED_SELECTOR, over_height)?.is_some() {
        return Err(
            "an overdrawn transfer recorded a Transferred event, the checked debit failed".into(),
        );
    }
    conservation(
        &client,
...
fn conservation(
    ...
    println!(
        "    [conservation {at}] supply={supply} {} sum={sum}",
        parts.join(" ")
    );
    if sum != supply {
        return Err(format!(
            "conservation broke {at}: sum of balances {sum} != supply {supply}"
        ));
    }
    Ok(())
}
